...
//第二组U4
sbit SCK4 = P6^1;//移位寄存器时钟，上升沿移位
sbit RCK4 = P0^7;//锁存寄存器时钟，高电平存储
sbit SER4 = P0^6;//串行数据输入端
void Hc595SendByte4(u8 dat);

//第二组U5
sbit SCK5 = P4^1;//移位寄存器时钟，上升沿移位
sbit RCK5 = P4^2;//锁存寄存器时钟，高电平存储
sbit SER5 = P6^2;//串行数据输入端
void Hc595SendByte5(u8 dat);

//第二组U6
sbit SCK6 = P6^0;//移位寄存器时钟，上升沿移位
